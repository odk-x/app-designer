window.odkCommonDefinitions = {
  "_tokens": {
    "select_user": {
      "string_token": "select_user",
      "text": {
        "default": "Please Select User",
        "greek": "Επιλέξτε Χρήστη",
        "arabic": "الرجاء اختيار المستخدم",
        "farsi": "لطفا انتخاب کنید کاربر",
        "spanish": "Por favor seleccione usuario"
      },
      "English": "Please Select User",
      "_row_num": 2
    },
    "main_title": {
      "string_token": "main_title",
      "text": {
        "default": "Disaster Relief",
        "greek": "Ανακούφιση από καταστροφές",
        "arabic": "توزيع الكوارث",
        "farsi": "تسکین فاجعه",
        "spanish": "Distribución"
      },
      "English": "Disaster Relief",
      "_row_num": 3
    },
    "registration_path": {
      "string_token": "registration_path",
      "text": {
        "default": "Registration",
        "greek": "Εγγραφή",
        "arabic": "التسجيل",
        "farsi": "ثبت",
        "spanish": "Registro"
      },
      "English": "Registration",
      "_row_num": 4
    },
    "delivery_path": {
      "string_token": "delivery_path",
      "text": {
        "default": "Delivery",
        "greek": "Διανομή",
        "arabic": "التسليم",
        "farsi": "تحویل",
        "spanish": "Entrega"
      },
      "English": "Delivery",
      "_row_num": 5
    },
    "data_path": {
      "string_token": "data_path",
      "text": {
        "default": "View Data",
        "greek": "Προβολή δεδομένων",
        "arabic": "راجع المعلومات",
        "farsi": "دیدن اطلاعات",
        "spanish": "Ver información"
      },
      "English": "View Data",
      "_row_num": 6
    },
    "override_path": {
      "string_token": "override_path",
      "text": {
        "default": "Administrator Overrides",
        "greek": "Διαχειριστής παρακάμπτει",
        "arabic": "مراقب التجاوزات",
        "farsi": "لغومدیر تنظیمات",
        "spanish": "Administrador Anulaciones"
      },
      "English": "Administrator Overrides",
      "_row_num": 7
    },
    "registration_title": {
      "string_token": "registration_title",
      "text": {
        "default": "Register Beneficiary Code",
        "greek": "Εγγραφή Κωδικού Δικαιούχου",
        "arabic": "تسجيل رمز المستفيد",
        "farsi": "ثبت نام کد ذینفع",
        "spanish": "Registrar código de beneficiario"
      },
      "English": "Register Beneficiary Code",
      "_row_num": 8
    },
    "delivery_title": {
      "string_token": "delivery_title",
      "text": {
        "default": "Verify Beneficiary Code",
        "greek": "Επαληθεύστε τον Κωδικό Δικαιούχου",
        "arabic": "تأكيد رمز المستفيد",
        "farsi": "تأیید کد ذینفع",
        "spanish": "Verificar código de beneficiario"
      },
      "English": "Verify Beneficiary Code",
      "_row_num": 9
    },
    "disabled_beneficiary_notification": {
      "string_token": "disabled_beneficiary_notification",
      "text": {
        "default": "This beneficiary is disabled",
        "greek": "Αυτός ο δικαιούχος είναι απενεργοποιημένος",
        "arabic": "هذا المستفيد معطل",
        "farsi": "این ذینفع غیر فعال است",
        "spanish": "Este beneficiario está deshabilitado"
      },
      "English": "This beneficiary is disabled",
      "_row_num": 10
    },
    "missing_beneficiary_notification": {
      "string_token": "missing_beneficiary_notification",
      "text": {
        "default": "Scanned code not found in system",
        "greek": "Ο σαρωμένος κωδικός δεν βρέθηκε στο σύστημα",
        "arabic": "ليس الرمز المصور موجودا في قاعدة البيانات",
        "farsi": "کد اسکن در سیستم یافت نشد",
        "spanish": "Código escaneado no encontrado en el sistema"
      },
      "English": "Scanned code not found in system",
      "_row_num": 11
    },
    "barcode_available": {
      "string_token": "barcode_available",
      "text": {
        "default": "Barcode Available",
        "greek": "Διαθέσιμος γραμμικός κώδικας",
        "arabic": "الباركود متاح",
        "farsi": "بارکد در دسترس",
        "spanish": "Código de barras disponible"
      },
      "English": "Barcode Available",
      "_row_num": 12
    },
    "voucher_detected": {
      "string_token": "voucher_detected",
      "text": {
        "default": "Voucher Barcode Detected",
        "greek": "Έγινε εντοπισμός του γραμμωτού κώδικα κουπονιών",
        "arabic": "تم قبول باركود الكرت",
        "farsi": "کوپن بارکد شناسایی",
        "spanish": "Detectado código de barras del cupón"
      },
      "English": "Voucher Barcode Detected",
      "_row_num": 13
    },
    "barcode_unavailable": {
      "string_token": "barcode_unavailable",
      "text": {
        "default": "Barcode Unavailable",
        "greek": "Barcode Μη διαθέσιμο",
        "arabic": "الباركود غير متاح",
        "farsi": "بارکد در دسترس",
        "spanish": "Código de barras no disponible"
      },
      "English": "Barcode Unavailable",
      "_row_num": 14
    },
    "already_qualifies_override": {
      "string_token": "already_qualifies_override",
      "text": {
        "default": "Scanned beneficiary already qualifies for this authorization",
        "greek": "Ο σαρωμένος δικαιούχος είναι ήδη κατάλληλος για αυτήν τη διανομή",
        "arabic": "المستفيد متاح للتوزيع",
        "farsi": "اسکن ذینفع در حال حاضر برای این توزیع",
        "spanish": "Beneficiario escaneado ya calificado para esta distribución"
      },
      "English": "Scanned beneficiary already qualifies for this distribution",
      "_row_num": 15
    },
    "eligible_override": {
      "string_token": "eligible_override",
      "text": {
        "default": "Beneficiary eligible for override",
        "greek": "Δικαιούχος για παράκαμψη",
        "arabic": "المستفيج مؤهل للتجاوز",
        "farsi": "لغو ذینفع واجد شرایط",
        "spanish": "Beneficiario apto para anulación"
      },
      "English": "Beneficiary eligible for override",
      "_row_num": 16
    },
    "override_creation_success": {
      "string_token": "override_creation_success",
      "text": {
        "default": "Override Successfully Created",
        "greek": "Η παράκαμψη δημιουργήθηκε με επιτυχία",
        "arabic": "تم التجاوز بنجاح",
        "farsi": "لغوموفق ثبت",
        "spanish": "Anulación creada satisfactoriamente"
      },
      "English": "Override Successfully Created",
      "_row_num": 17
    },
    "view_authorization_details": {
      "string_token": "view_authorization_details",
      "text": {
        "default": "View Distribution Details",
        "greek": "Προβολή λεπτομερειών διανομής",
        "arabic": "انظر تفاصيل التوزيع",
        "farsi": "جزئیات نمایش توزیع",
        "spanish": "Ver detalles de la distribución"
      },
      "English": "View Distribution Details",
      "_row_num": 18
    },
    "scan_barcode": {
      "string_token": "scan_barcode",
      "text": {
        "default": "Scan Barcode",
        "greek": "Σάρωση γραμμωτού κώδικα",
        "arabic": "مسح الباركود",
        "farsi": "بارکد اسکن",
        "spanish": "Escanear código de barras"
      },
      "English": "Scan Barcode",
      "_row_num": 19
    },
    "enter": {
      "string_token": "enter",
      "text": {
        "default": "Enter",
        "greek": "Εισάγω",
        "arabic": "تنفيد",
        "farsi": "وارد شدن",
        "spanish": "Indroducir"
      },
      "English": "Enter",
      "_row_num": 20
    },
    "no_active_beneficiary": {
      "string_token": "no_active_beneficiary",
      "text": {
        "default": "No Active Beneficiary Detected",
        "greek": "Δεν εντοπίστηκε Ενεργός Δικαιούχος",
        "arabic": "لم يكشف مستفيد موجود",
        "farsi": "فعال بدون بهره  یافت شد",
        "spanish": "Detectado beneficiario no activo"
      },
      "English": "No Active Beneficiary Detected",
      "_row_num": 21
    },
    "no_disabled_beneficiary": {
      "string_token": "no_disabled_beneficiary",
      "text": {
        "default": "No Disabled Beneficiary Detected",
        "greek": "Δεν εντοπίστηκε δικαιούχος με αναπηρία",
        "arabic": "لم يكشف مستفيد معطل",
        "farsi": "غیر فعال بدون بهره یافت شد",
        "spanish": "Detectado beneficiario no desactivado"
      },
      "English": "No Disabled Beneficiary Detected",
      "_row_num": 22
    },
    "enter_beneficiary_code": {
      "string_token": "enter_beneficiary_code",
      "text": {
        "default": "Please Enter Beneficiary Code",
        "greek": "Εισαγάγετε τον Κωδικό Δικαιούχου",
        "arabic": "الرجاء إدخال رمز المستفيد",
        "farsi": "لطفا کد ذینفع را وارد کنید",
        "spanish": "Por favor introduce el código del beneficiario"
      },
      "English": "Please Enter Beneficiary Code",
      "_row_num": 23
    },
    "active_beneficiaries_title": {
      "string_token": "active_beneficiaries_title",
      "text": {
        "default": "Active Beneficiaries",
        "greek": "Ενεργοί Δικαιούχοι",
        "arabic": "المستفيدين النشطين",
        "farsi": "ذینفع فعال",
        "spanish": "Beneficiarios activos"
      },
      "English": "Active Beneficiaries",
      "_row_num": 24
    },
    "disabled_beneficiaries_title": {
      "string_token": "disabled_beneficiaries_title",
      "text": {
        "default": "Disabled Beneficiaries",
        "greek": "Οι δικαιούχοι με ειδικές ανάγκες",
        "arabic": "المستفيدون المعطلين",
        "farsi": "ذینفع غیر فعال",
        "spanish": "Beneficiarios desactivados"
      },
      "English": "Disabled Beneficiaries",
      "_row_num": 25
    },
    "beneficiary_data_title": {
      "string_token": "beneficiary_data_title",
      "text": {
        "default": "Beneficiary Data",
        "greek": "Δεδομένα Δικαιούχου",
        "arabic": "معلومات المستفيد",
        "farsi": "اطلاعات ذینفع",
        "spanish": "Datos de beneficiarios"
      },
      "English": "Beneficiary Data",
      "_row_num": 26
    },
    "delivery_data_title": {
      "string_token": "delivery_data_title",
      "text": {
        "default": "Delivery Data",
        "greek": "Δεδομένα παράδοσης",
        "arabic": "معلومات التسليم",
        "farsi": "اطلاعات تحویل",
        "spanish": "Datos de entrega"
      },
      "English": "Delivery Data",
      "_row_num": 27
    },
    "beneficiary_lists": {
      "string_token": "beneficiary_lists",
      "text": {
        "default": "Beneficiary Lists",
        "greek": "Κατάλογος δικαιούχων",
        "arabic": "قأئمة المستفيدين",
        "farsi": "لیست ذینفع",
        "spanish": "Lista de beneficiarios"
      },
      "English": "Beneficiary Lists",
      "_row_num": 28
    },
    "view_all_deliveries": {
      "string_token": "view_all_deliveries",
      "text": {
        "default": "View All Deliveries",
        "greek": "Προβολή όλων των παραδόσεων",
        "arabic": "راقب كل التسليمات",
        "farsi": "مشاهده تحویل",
        "spanish": "Ver todas las entregas"
      },
      "English": "View All Deliveries",
      "_row_num": 29
    },
    "advanced_search": {
      "string_token": "advanced_search",
      "text": {
        "default": "Advanced Search",
        "greek": "Προχωρημένη Αναζήτηση",
        "arabic": "البحث المتقدم",
        "farsi": "جستجوی پیشرفته",
        "spanish": "Búsqueda avanzada"
      },
      "English": "Advanced Search",
      "_row_num": 30
    },
    "data_categories": {
      "string_token": "data_categories",
      "text": {
        "default": "Data Categories",
        "greek": "Κατηγορίες δεδομένων",
        "arabic": "نوعية المعلومات",
        "farsi": "دسته بندی اطلاعات",
        "spanish": "Categorías de datos"
      },
      "English": "Data Categories",
      "_row_num": 31
    },
    "view_beneficiary_data": {
      "string_token": "view_beneficiary_data",
      "text": {
        "default": "View Beneficiary Data",
        "greek": "Προβολή δεδομένων δικαιούχου",
        "arabic": "انظر معلومات المستفيد",
        "farsi": "دیدن اطلاعات ذینفع",
        "spanish": "Ver datos del beneficiario"
      },
      "English": "View Beneficiary Data",
      "_row_num": 32
    },
    "view_delivery_data": {
      "string_token": "view_delivery_data",
      "text": {
        "default": "View Delivery Data",
        "greek": "Προβολή δεδομένων παράδοσης",
        "arabic": "انظر معلومات التسليمات",
        "farsi": "دیدن اطلاعات  تحویل",
        "spanish": "Ver datos de entregas"
      },
      "English": "View Delivery Data",
      "_row_num": 33
    },
    "enable_beneficiary": {
      "string_token": "enable_beneficiary",
      "text": {
        "default": "Enable Beneficiary",
        "greek": "Ενεργοποίηση του Δικαιούχου",
        "arabic": "تمكين المستفيد",
        "farsi": "فعال کردن  ذینفع",
        "spanish": "Habilitar beneficiario"
      },
      "English": "Enable Beneficiary",
      "_row_num": 34
    },
    "disable_beneficiary": {
      "string_token": "disable_beneficiary",
      "text": {
        "default": "Disable Beneficiary",
        "greek": "Απενεργοποιήστε τον Δικαιούχο",
        "arabic": "تعطيل المستفيدين",
        "farsi": "غیر فعال کردن  ذینفع",
        "spanish": "Deshabilitar beneficiario"
      },
      "English": "Disable Beneficiary",
      "_row_num": 35
    },
    "choose_method": {
      "string_token": "choose_method",
      "text": {
        "default": "Choose Method",
        "greek": "Επιλέξτε Μέθοδο",
        "arabic": "اختار المنهح",
        "farsi": "انتخاب روش",
        "spanish": "Elegir método"
      },
      "English": "Choose Method",
      "_row_num": 36
    },
    "override_options": {
      "string_token": "override_options",
      "text": {
        "default": "Override Options",
        "greek": "Επιλογές παράκαμψης",
        "arabic": "اختيارات التجاوز",
        "farsi": "لغو گزینه",
        "spanish": "Anular opciones"
      },
      "English": "Override Options",
      "_row_num": 37
    },
    "override_registration": {
      "string_token": "override_registration",
      "text": {
        "default": "Override Registration",
        "greek": "Αντικατάσταση εγγραφής",
        "arabic": "اختيارات التسجيل",
        "farsi": "لغوثبت نام",
        "spanish": "Anular registro"
      },
      "English": "Override Registration",
      "_row_num": 38
    },
    "override_entitlement": {
      "string_token": "override_entitlement",
      "text": {
        "default": "Override Entitlement",
        "greek": "Παράκαμψη του Δικαιώματος",
        "arabic": "اختيارات الاستحقاق",
        "farsi": "لغواستحقاق",
        "spanish": "Anular título"
      },
      "English": "Override Entitlement",
      "_row_num": 39
    },
    "search_beneficiaries": {
      "string_token": "search_beneficiaries",
      "text": {
        "default": "Search for Beneficiaries",
        "greek": "Αναζήτηση δικαιούχων",
        "arabic": "ابحث عن مستفيدين",
        "farsi": "جستجو برای ذینفع",
        "spanish": "Buscar beneficiarios"
      },
      "English": "Search for Beneficiaries",
      "_row_num": 40
    },
    "search_deliveries": {
      "string_token": "search_deliveries",
      "text": {
        "default": "Search for Deliveries",
        "greek": "Αναζήτηση για παραδόσεις",
        "arabic": "ابحث عن تسليمات",
        "farsi": "جستجو برای تحویل",
        "spanish": "Buscar entregas"
      },
      "English": "Search for Deliveries",
      "_row_num": 41
    },
    "beneficiary": {
      "string_token": "beneficiary",
      "text": {
        "default": "beneficiary",
        "greek": "δικαιούχος",
        "arabic": "مستفيد",
        "farsi": "ذینفع",
        "spanish": "Beneficiario/a"
      },
      "English": "beneficiary",
      "_row_num": 42
    },
    "beneficiaries": {
      "string_token": "beneficiaries",
      "text": {
        "default": "beneficiaries",
        "greek": "Δικαιούχοι",
        "arabic": "مستفيدين",
        "farsi": "ذینفع",
        "spanish": "Beneficiarios/as"
      },
      "English": "beneficiaries",
      "_row_num": 43
    },
    "delivery": {
      "string_token": "delivery",
      "text": {
        "default": "delivery",
        "greek": "διανομή",
        "arabic": "تسليم",
        "farsi": "تحویل",
        "spanish": "Entrega"
      },
      "English": "delivery",
      "_row_num": 44
    },
    "deliveries": {
      "string_token": "deliveries",
      "text": {
        "default": "deliveries",
        "greek": "Παραδόσεις",
        "arabic": "تسليمات",
        "farsi": "تحویل",
        "spanish": "Entregas"
      },
      "English": "deliveries",
      "_row_num": 45
    },
    "found": {
      "string_token": "found",
      "text": {
        "default": "found",
        "greek": "βρέθηκαν",
        "arabic": "موجود",
        "farsi": "مکتشف",
        "spanish": "Encontrado"
      },
      "English": "found",
      "_row_num": 46
    },
    "invalid_search": {
      "string_token": "invalid_search",
      "text": {
        "default": "Invalid Search",
        "greek": "Μη έγκυρη αναζήτηση",
        "arabic": "البحث غير صحيح",
        "farsi": "جستجو نامعتبر",
        "spanish": "Búsqueda inválida"
      },
      "English": "Invalid Search",
      "_row_num": 47
    },
    "authorization_name": {
      "string_token": "authorization_name",
      "text": {
        "default": "Distribution Name",
        "greek": "Όνομα διανομής",
        "arabic": "اسم التوزيع",
        "farsi": "توزیع نام",
        "spanish": "Nombre de la distribución"
      },
      "English": "Distribution Name",
      "_row_num": 48
    },
    "authorization_id": {
      "string_token": "authorization_id",
      "text": {
        "default": "Distribution ID",
        "greek": "Αναγνωριστικό διανομής",
        "arabic": "رقم التوزيع",
        "farsi": "شماره توزیع",
        "spanish": "ID de la distribución"
      },
      "English": "Distribution ID",
      "_row_num": 49
    },
    "item_pack_name": {
      "string_token": "item_pack_name",
      "text": {
        "default": "Item Pack Name",
        "greek": "Όνομα πακέτου στοιχείου",
        "arabic": "اسم الحزمة",
        "farsi": "نام بسته",
        "spanish": "Nombre del artículo"
      },
      "English": "Item Pack Name",
      "_row_num": 50
    },
    "item_pack_description": {
      "string_token": "item_pack_description",
      "text": {
        "default": "Item Pack Description",
        "greek": "Περιγραφή πακέτου στοιχείου",
        "arabic": "وصف الحزمة",
        "farsi": "شرح بسته",
        "spanish": "Descripción del artículo"
      },
      "English": "Item Pack Description",
      "_row_num": 51
    },
    "item_pack_id": {
      "string_token": "item_pack_id",
      "text": {
        "default": "Item Pack ID",
        "greek": "Αναγνωριστικό πακέτου στοιχείου",
        "arabic": "رقم الحزمة",
        "farsi": "شماره بسته",
        "spanish": "ID del artículo"
      },
      "English": "Item Pack ID",
      "_row_num": 52
    },
    "beneficiary_code": {
      "string_token": "beneficiary_code",
      "text": {
        "default": "Beneficiary Code",
        "greek": "Κωδικός Δικαιούχου",
        "arabic": "رمز المستفيد",
        "farsi": "کد ذینفع",
        "spanish": "Código de beneficiario"
      },
      "English": "Beneficiary Code",
      "_row_num": 53
    },
    "no_authorizations": {
      "string_token": "no_authorizations",
      "text": {
        "default": "No Distributions",
        "greek": "Καμία διανομή",
        "arabic": "لا توزيعات",
        "farsi": "بدون توزیع",
        "spanish": "Sin distribuciones"
      },
      "English": "No Distributions",
      "_row_num": 54
    },
    "view_household_data": {
      "string_token": "view_household_data",
      "text": {
        "default": "View Household Data"
      },
      "_row_num": 55
    },
    "active_households": {
      "string_token": "active_households",
      "text": {
        "default": "Active Households"
      },
      "_row_num": 56
    },
    "disabled_households": {
      "string_token": "disabled_households",
      "text": {
        "default": "Disabled Households"
      },
      "_row_num": 57
    },
    "household_data": {
      "string_token": "household_data",
      "text": {
        "default": "Household Data"
      },
      "_row_num": 58
    },
    "search_members": {
      "string_token": "search_members",
      "text": {
        "default": "Beneficiary Advanced Search"
      },
      "_row_num": 59
    },
    "search_households": {
      "string_token": "search_households",
      "text": {
        "default": "Household Advanced Search"
      },
      "_row_num": 60
    }
  },
  "_locales": {
    "setting_name": "_locales",
    "_row_num": 5,
    "value": [
      {
        "display": {
          "locale": {
            "text": {
              "english": "English",
              "spanish": "Ingles",
              "greek": "English",
              "arabic": "English",
              "farsi": "English",
              "default": "English"
            }
          }
        },
        "_row_num": 7,
        "name": "english"
      },
      {
        "display": {
          "locale": {
            "text": {
              "english": "Spanish",
              "spanish": "Espanol",
              "greek": "Espanol",
              "arabic": "Spanish",
              "farsi": "Spanish",
              "default": "Spanish"
            }
          }
        },
        "_row_num": 8,
        "name": "spanish"
      },
      {
        "display": {
          "locale": {
            "text": {
              "english": "Arabic",
              "spanish": "Arabic",
              "greek": "Arabic",
              "arabic": "Arabic",
              "farsi": "Arabic",
              "default": "Arabic"
            }
          }
        },
        "_row_num": 9,
        "name": "arabic"
      },
      {
        "display": {
          "locale": {
            "text": {
              "english": "Farsi",
              "spanish": "Farsi",
              "greek": "Farsi",
              "arabic": "Farsi",
              "farsi": "Farsi",
              "default": "Farsi"
            }
          }
        },
        "_row_num": 10,
        "name": "farsi"
      },
      {
        "display": {
          "locale": {
            "text": {
              "english": "Greek",
              "spanish": "Greek",
              "greek": "Greek",
              "arabic": "Greek",
              "farsi": "Greek",
              "default": "Greek"
            }
          }
        },
        "_row_num": 11,
        "name": "greek"
      }
    ]
  },
  "_default_locale": {
    "setting_name": "_default_locale",
    "_row_num": 5,
    "value": "english"
  }
}