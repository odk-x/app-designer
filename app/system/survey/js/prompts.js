...
promptTypes.base = Backbone.View.extend({
     ...
     * the Excel worksheet.
     */
    constraint_message: odkCommon.lookupToken('constraint_message'),
    invalid_value_message: odkCommon.lookupToken('invalid_value_message'),
    required_message: odkCommon.lookupToken('required_message'),
	required_exception_message: odkCommon.lookupToken('required_exception_message'),
	constraint_exception_message: odkCommon.lookupToken('constraint_exception_message'),
	...
	_foldInProtoDisplayFields: function() {
		...
		if ( 'display' in that ) {
			...
			if ( '_protoDisplay' in that ) {
				for (var property in that._protoDisplay) {
					if (that._protoDisplay.hasOwnProperty(property)) {
						if ( !(property in displayField ) ) {
							displayField[property] = that._protoDisplay[property];
						}
					}
				}
			}
    ...
    _isValid: function(isStrict) {
        ...
        try {
            ...
            if ( isStrict ) {
                var localizeRequiredExceptionMessage = formulaFunctions.localize(locale, that.required_exception_message);
        ...
        if ( value === undefined || value === null || value === "" ) {
            if ( isRequired ) {
                that.valid = false;
                var localizeRequiredMessage = formulaFunctions.localize(locale, that.required_message);
                return { message: localizeRequiredMessage + " " + fieldDisplayName };
            }
        } else if ( 'validateValue' in that ) {
            if ( !that.validateValue() ) {
                that.valid = false;
                var localizeInvalidValueMessage = formulaFunctions.localize(locale, that.invalid_value_message);
        ...
        if ( 'constraint' in that ) {
            ...
            try {
                ...
                if ( !outcome ) {
                    that.valid = false;
                    var localizeConstraintMessage = formulaFunctions.localize(locale, that.constraint_message);
                    return { message: localizeConstraintMessage + " " + fieldDisplayName };
                }
            } catch (e) {
                var localizeConstraintExceptionMessage = formulaFunctions.localize(locale, that.constraint_exception_message);
