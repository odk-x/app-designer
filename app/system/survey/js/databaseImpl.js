...
return {
  ...
  withDb:function(ctxt, transactionBody) {
    ...
    ctxt.log('D','databaseImpl.withDb');
    ...
    try {
        if ( dbif.w3cSqlIsOpen() ) {
            ...
            ctxt.log('E','databaseImpl.withDb.notSupported w3c SQL interface is not supported');
            inContinuation = true;
...
putDataKeyValueMap:function(ctxt, model, formId, instanceId, kvMap) {
    ctxt.log('D','databaseImpl.putDataKeyValueMap.entered');
    if ($.isEmptyObject(kvMap)) {
        ctxt.success();
          ...
          ctxt.log('D','databaseImpl.putDataKeyValueMap.initiated', names );

          var updates = {};
          var tmpctxt = $.extend({}, ctxt, {success:function() {
                    ctxt.log('D','databaseImpl.putDataKeyValueMap.updatingCache');
...
loadDataKeyValueMap:function(ctxt, model, formId, kvMap) {
    ...
    if ($.isEmptyObject(kvMap)) {
        ctxt.success();
          ...
          names = names.substring(1);
          ctxt.log('D','databaseImpl.loadDataKeyValueMap.initiated', names );

          var updates = {};
          var tmpctxt = $.extend({}, ctxt, {success:function() {
                    ctxt.log('D','databaseImpl.loadDataKeyValueMap.updatingCache');
                    ctxt.success();
                }});
...
ignore_all_changes:function(ctxt, model, formId, instanceId) {
      var that = this;
      ctxt.log('I','databaseImpl.ignore_all_changes');
      that.withDb( ctxt, function(transaction) {
...
delete_checkpoints_and_row:function(ctxt, dbTableName, instanceId) {
      var that = this;
      ctxt.log('I','delete_checkpoints_and_row');
      that.withDb( ctxt, function(transaction) {
...
initializeInstance:function(ctxt, instanceId, instanceMetadataKeyValueMap) {
    ...
    if ( instanceId === undefined || instanceId === null ) {
        ...
        var tmpctxt = $.extend({},ctxt,{success:function() {
                that.getAllData(ctxt, model, instanceId);
            }});
        that.withDb( tmpctxt, function(transaction) {
                ...
                transaction.executeSql(ss.stmt, ss.bind, function(transaction, result) {
                    ...
                    if (len === 0 ) {
						if (opendatakit.getPlatformInfo().container === 'Android') {
							throw new Error("getMetaData: table is not defined! " + name);
						} else {
...
_insertTableAndColumnProperties:function(transaction, ctxt, tlo, writeDatabase) {
    var that = this;
    ctxt.log('D','databaseImpl._insertTableAndColumnProperties writeDatabase: ' + writeDatabase);
    var fullDef = 
...
purge:function(ctxt) {
    var that = this;
    ctxt.log('I','databaseImpl.purge.initiated');
    var tableSets = [];
    that.withDb( $.extend({},ctxt,{success:function() {
