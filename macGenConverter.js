var XRegExp = require("./xlsxconverter/XRegExp-All-4.4.0.js");
...
function writeOutProp(formDefStr) {
    ...
    try {
        if (util.shouldWriteOutDefinitionAndPropertiesCsv(formDefJson)) {
            ...
            var propCsv = util.createPropertiesCsvFromDataTableModel(
                dtm,
                formDefJson
            );
    ...
    try {
        if (util.shouldWriteOutDefinitionsJs(formDefJson)) {
            if (tableId === "framework") {
                ...
                var trxPath =
                    util.getRelativePathToFrameworkDefinitionsJs(formDefJson);
                var trxJs = util.createDefinitionsJsFromDataTableModel(
                    tableId,
                    formDefJson
                );
                ...
                var trxCmnPath =
                    util.getRelativePathToCommonDefinitionsJs(formDefJson);
                var trxCmnJs = util.createDefinitionsJsFromDataTableModel(
                    null,
                    formDefJson
                );
        ...
        throw new Error(
            "unable to write out ...Definitions file: " +
                tableId +
                " error: " +
                e.name +
                " message: " +
