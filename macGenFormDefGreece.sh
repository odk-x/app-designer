...
if [ "$1" ]; then    
    ...
    do
        ...
        node macGenConverterGreece.js $f >> $TEMP2$TEMP
    done   
else
    for f in appGreece/config/tables/*/forms/*/*.xlsx
    do
        x=$f
        P=${x%.xlsx}
        FILENAME="${P##*/}"
    ...
    for f in appGreece/config/assets/framework/forms/framework/framework.xlsx
    do
