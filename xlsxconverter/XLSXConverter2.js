...
(function (XLSXConverter) {
    ...
    if (typeof exports !== "undefined") {
        if (typeof require !== "undefined")
            XRegExp = require("./XRegExp-All-4.4.0.js");
    }
    var pattern_valid_user_defined_name = XRegExp(
    ...
    var default_initial = [
        ...
        { _row_num: 3, type: "opening", display: { text: "Edit form" } },
        { _row_num: 4, clause: "do section survey" },
        { _row_num: 5, type: "finalize", display: { text: "Save form" } },
        { _row_num: 6, clause: "else // start" },
        {
            _row_num: 7,
            type: "instances",
            display: { text: "Saved instances" },
        },
        { _row_num: 8, clause: "end if // start" },
    ];
                    ...
                    if (prop in obj) {
                        ...
                        if (i !== -1) {
                            if (prop.lastIndexOf("]") !== prop.length - 1) {
                                throw Error(
                                    "Invalid array subscript in column heading: " +
                                        prop
                                );
                            }
    ...
    */
    var groupColumnHeaders = function (row) {
        ...
        _.each(row, function (value, columnHeader) {
            ...
            outRow = recursiveExtend(
                outRow,
                listToNestedDict(chComponents.concat(value))
            );
        });
        return outRow;
    };
    ...
    var errorIfFieldMissing = function (
        sheetName,
        rows,
        requiredField,
        nonEmpty
        ...
        _.each(rows, function (row) {
            ...
            if (
                ...
                throw Error(
                    "Cell value is unexpectedly empty on sheet: " +
                        sheetName +
                        " for column: " +
                        requiredField +
                        " on row: " +
    ...
    var errorIfFieldsMissingForQueryType = function (
        sheetName,
        rows,
        queryType,
        requiredField
    ...
    var parseControlFlowSection = function (sheetName, sheet) {
        ...
        _.each(sheet, function (row) {
            ...
            if ("clause" in row) {
                ...
                if (first.length >= 2) {
                    if (first.substring(0, 2) == "//") first = "comment";
                }
                ...
                switch (first) {
                    case "begin":
                        if (
                            ...
                            throw Error(
                                "Expected 'begin screen [ // <tagname> ]' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    case "end":
                        if (
                            parts.length < 2 ||
                            (parts[1] !== "screen" && parts[1] !== "if")
                        ) {
                            throw Error(
                                "Expected 'end if' or 'end screen' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                        ...
                        if ("condition" in row) {
                            throw Error(
                                "'condition' expressions are not allowed on 'end " +
                                    parts[1] +
                                    "' clauses. Error on sheet: " +
                                    sheetName +
                                    " on row: " +
                                    row._row_num
                            );
                    ...
                    case "if":
                        ...
                        if (!("condition" in row)) {
                            throw Error(
                                "'condition' expression is required on 'if' clauses. Error on sheet: " +
                                    sheetName +
                                    " on row: " +
                                    row._row_num
                            );
                    ...
                    case "else":
                        if (
                            ...
                            throw Error(
                                "Expected 'else [ // <tagname> ]' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    case "goto":
                        if (parts.length !== 2) {
                            throw Error(
                                "Expected 'goto <branchlabel>' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    case "do":
                        if (parts.length !== 3 || parts[1] !== "section") {
                            throw Error(
                                "Expected 'do section <sectionname>' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    case "exit":
                        if (parts.length !== 2 || parts[1] !== "section") {
                            throw Error(
                                "Expected 'exit section' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    case "save":
                        if (
                            ...
                            throw Error(
                                "Expected 'save and terminate' but found: " +
                                    row.clause +
                                    " on sheet: " +
                                    sheetName +
                                    " on row: " +
                    ...
                    default:
                        throw Error(
                            "Unrecognized 'clause' expression: " +
                                row.clause +
                                " on sheet: " +
                                sheetName +
                                " on row: " +
                ...
                if ("condition" in row) {
                    throw Error(
                        "'condition' expressions are not allowed on prompts. Error on sheet: " +
                            sheetName +
                            " on row: " +
                            row._row_num
                    );
                ...
                if (first === "assign") {
                    ...
                    if (!("name" in row)) {
                        throw Error(
                            "'assign' expressions must specify a field 'name' Error on sheet: " +
                                sheetName +
                                " on row: " +
                                row._row_num
                        );
    ...
    var parseScreenBlock = function (sheetName, flow, idx) {
        ...
        while (i < flow.length) {
            ...
            switch (clause._token_type) {
                ...
                case "else":
                    throw Error(
                        "'else' without preceding 'if' clause: '" +
                            clause.clause +
                            "' at row " +
                            clause._row_num +
                            " on sheet: " +
                ...
                case "end_if":
                    throw Error(
                        "'end if' without preceding 'if' clause: '" +
                            clause.clause +
                            "' at row " +
                            clause._row_num +
                            " on sheet: " +
        ...
        if (tag !== undefined && tag !== null) {
            ...
            throw Error(
                "No matching 'end screen' on sheet: " +
                    sheetName +
                    " for '" +
                    flow[idx].clause +
                    "' at row " +
    ...
    var parseTopLevelIfBlock = function (sheetName, flow, idx) {
        ...
        while (i < flow.length) {
            ...
            switch (clause._token_type) {
                ...
                case "end_screen":
                    throw Error(
                        "'end screen' without preceding 'begin screen' clause: '" +
                            clause.clause +
                            "' at row " +
                            clause._row_num +
                            " on sheet: " +
     ...
     * Map the
     */
    var updateValidationTagMap = function (
        validationTagMap,
        promptIdx,
        clause
    ) {
    ...
    var constructScreenDefn = function (
        sheetName,
        prompts,
        validationTagMap,
        blockFlow,
        idx,
        enclosingScreenLabel,
        enclosing_row_num
    ) {
        if (
            enclosingScreenLabel === undefined ||
            enclosingScreenLabel === null
        ) {
            throw Error(
                "Internal error. No enclosing screen label defined on sheet: " +
                    sheetName +
                    " at row " +
                    enclosing_row_num
            );
        ...
        while (i < blockFlow.length) {
            ...
            switch (clause._token_type) {
                ...
                case "prompt":
                    ...
                    clause._branch_label_enclosing_screen =
                        sheetName + "/" + enclosingScreenLabel;
        ...
        while (i < blockFlow.length) {
            ...
            switch (clause._token_type) {
                ...
                case "begin_if":
                    ...
                    var elseClause =
                        clause._else_clause !== undefined &&
                        clause._else_clause !== null
                            ? clause._else_clause
                            : endIfClause;
                    var thenBlock = clause._then_block;
                    var elseBlock = clause._else_block;
     ...
     */
    var extractBranchLabels = function (
        sheetName,
        operations,
        branchLabelMap,
        flattened,
     ...
     */
    var verifyReachableLabels = function (
        sheetName,
        operations,
        branchLabelMap,
        idx,
     ...
     */
    var gatherNestedSections = function (
        sheetName,
        operations,
        nestedSections,
        idx
     ...
     */
    var sanityCheckOneField = function (
        field,
        columnType,
        worksheet,
        rowNum,
        ...
        if (_.isArray(columnType)) {
            throw Error(
                "Unable to handle array-valued column_types enforcement: ",
                columnPath
            );
        }
        ...
        if (
            columnType === "formula" ||
            columnType.substring(0, formulaArgs.length) == formulaArgs
        ) {
     ...
     */
    var sanityCheckFieldTypes = function (
        rowObject,
        worksheet,
        specColumnTypes
    ) {
    ...
    var parseSections = function (
        ...
        _.each(sections, function (section, sectionName) {
            var sectionObject = parseSection(
                sectionName,
                section,
                specSettings,
                specColumnTypes
        ...
        for (i = 1; i <= base; i = 2 * i) {
            _.each(newSections, function (section) {
                ...
                _.each(
                    ...
                    function (reached, sectionName) {
                        newReachable[sectionName] = true;
                        if (newSections[sectionName]) {
                            _.each(
                                newSections[sectionName].reachable_sections,
                                function (rreached, depthOne) {
                                    newReachable[depthOne] = true;
                                }
                            );
                        } else {
                            throw new Error(
                                "Cannot convert section " +
                                    sectionName +
                                    ". Section may be blank or missing."
                            );
                        }
                    }
                );
        ...
        if (key_length === 1 && "_finalize" in keys) {
            ...
            _.each(newSections, function (section) {
                section.validation_tag_map.finalize =
                    section.validation_tag_map._finalize;
    ...
    var updateModel = function (section, promptOrAction, model, schema) {
        ...
        if (name in model) {
            defn = model[name];
            var amodb = deepExtendObject(
                deepExtendObject(deepExtendObject({}, defn), schema),
                mdef
            );
            var bmoda = deepExtendObject(
                deepExtendObject(deepExtendObject({}, schema), mdef),
                defn
            );
            ...
            defn._defn.push({
                _row_num: promptOrAction._row_num,
    ...
    var assertValidUserDefinedName = function (errorPrefix, name) {
        ...
        if (!pattern_valid_user_defined_name.test(name)) {
            throw Error(
                errorPrefix +
                    " must begin with a letter and contain only letters, digits and '_'"
            );
        }
        ...
        if (name.length > 59) {
            throw Error(
                errorPrefix +
                    " cannot be longer than 59 characters. It is " +
                    name.length +
                    " characters long."
            );
        }
        ...
        if (lowercase in reservedFieldNames) {
            throw Error(
                errorPrefix +
                    " is one of a long list of reserved words. To work around these, consider using a concatenation of two words (e.g., 'firstname')."
            );
        }
    };
     ...
     */
    var assertValidElementKey = function (
        elementPath,
        elementKey,
        fullSetOfElementKeys
    ) {
        var isSimple = false;
        if (elementPath === undefined || elementPath === null) {
            ...
            try {
                ...
                var ex1name = elementPath.substring(
                    0,
                    elementPath.indexOf(".")
                );
                throw Error(
                    e.message +
                        " Full elementPath: " +
                        elementPath +
                        " Consider shortening the field name '" +
                        ex1name +
                        "'."
                );
            }
        ...
        if (
            fullSetOfElementKeys !== undefined &&
            fullSetOfElementKeys !== null
        ) {
            if (elementKey in fullSetOfElementKeys) {
                ...
                if (isSimple) {
                    ...
                    throw Error(
                        "The framework's identifier for field '" +
                            elementPath +
                            "' collides with the identifier for the full elementPath: '" +
                            path +
                            "'. Please change one of the field names ('" +
                            elementPath +
                            "' or '" +
                            ex2name +
                            "')."
                    ...
                    var refname =
                        path.indexOf(".") === -1
                            ? path
                            : path.substring(0, path.indexOf("."));
                    var ex3name = elementPath.substring(
                        0,
                        elementPath.indexOf(".")
                    );
                    throw Error(
                        "The framework's identifier for the full elementPath: '" +
                            elementPath +
                            "' collides with the identifier for the full elementPath: '" +
                            path +
                            "'. Please change one of the field names ('" +
    ...
    var assignItemsElementKey = function (
        elementPathPrefix,
        elementKeyPrefix,
        fullSetOfElementKeys,
        enclosingElement
        ...
        if (
            "elementKey" in enclosingElement.items &&
            enclosingElement.items.elementKey != key
        ) {
            throw Error(
                "elementKey is user-defined for '" +
                    elementPath +
                    "' (the array-element type declaration) but does not match computed key: " +
                    key
            );
        }
     ...
     */
    var recursiveAssignPropertiesElementKey = function (
        elementPathPrefix,
        elementKeyPrefix,
        fullSetOfElementKeys,
        properties
        ...
        _.each(_.keys(properties), function (name) {
            ...
            if ("elementKey" in entry && entry.elementKey != key) {
                throw Error(
                    "elementKey is user-defined for " +
                        elementPath +
                        " but does not match computed key: " +
                        key
                );
            }
            assertValidElementKey(elementPath, key, fullSetOfElementKeys);
            ...
            if (entry.type === "object") {
                // we have properties...
                recursiveAssignPropertiesElementKey(
                    elementPath,
                    key,
                    fullSetOfElementKeys,
                    entry.properties
    ...
    var recursivelyResolveTypeInDataModel = function (term, promptTypes) {
        if ("properties" in term) {
            _.each(_.keys(term.properties), function (name) {
                ...
                if ("type" in defn) {
                    ...
                    if (type in promptTypes) {
                        ...
                        if (schema) {
                            ...
                            delete defn.type;
                            term.properties[name] = deepExtendObject(
                                schema,
                                defn
                            );
                        }
                ...
                recursivelyResolveTypeInDataModel(
            ...
            if ("type" in defn) {
                ...
                if (type in promptTypes) {
                    ...
                    if (schema) {
                        ...
                        // override the promptTypes 'type' with info supplied by the prompt_type sheet.
                        delete defn.type;
                        specification.model[name] = deepExtendObject(
                            schema,
                            defn
                        );
                    }
            ...
            _.each(section.prompts, function (prompt) {
                ...
                if (prompt._type in promptTypes) {
                    ...
                    if (schema) {
                        ...
                        throw Error(
                            "Unrecognized prompt type. Prompt: '" +
                                prompt.type +
                                "' at row " +
                                prompt._row_num +
                                " on sheet: " +
            ...
            _.each(section.operations, function (operation) {
                if (operation._token_type === "assign") {
                    if ("name" in operation) {
                        ...
                        if (
                            operation._data_type === undefined ||
                            operation._data_type === null
                        ) {
        ...
        _.each(_.keys(specification.model), function (name) {
            if (name in model) {
                ...
                if (!_.isEqual(amodb, bmoda)) {
                    ...
                    warnings.warn(
                        name +
                            " has different definitions at row " +
                            mdef._defn[0]._row_num +
                            " on " +
                            mdef._defn[0].section_name +
                            " sheet than " +
                            formatted.substring(2) +
                            " " +
                            mdef._defn[0].section_name +
        ...
        for (var i = 0; i < assigns.length; ++i) {
            ...
            if (!(operation.name in model)) {
                throw Error(
                    "Field name '" +
                        operation.name +
                        "' does not have a defined storage type. Clause: '" +
                        operation.type +
                        "' at row " +
                        operation._row_num +
                        " on sheet: " +
    ...
    var _setSessionVariableFlag = function (dbKeyMap, listChildElementKeys) {
        ...
        var i;
        if (
            listChildElementKeys !== undefined &&
            ...
            for (i = 0; i < listChildElementKeys.length; ++i) {
                ...
                if (jsonType.type === "array") {
                    _setSessionVariableFlag(
                        dbKeyMap,
                        jsonType.listChildElementKeys
                    );
     ...
     */
    var flattenElementPath = function (
        dbKeyMap,
        elementPathPrefix,
        elementName,
        elementKeyPrefix,
        ...
        elementPathPrefix =
            elementPathPrefix === undefined || elementPathPrefix === null
                ? elementName
        ...
        if (elementKey === undefined || elementKey === null) {
            throw new Error(
                "elementKey is not defined for '" + jsonType.elementPath + "'."
            );
        }
        ...
        if (
            dbKeyMap[elementKey] !== undefined &&
            dbKeyMap[elementKey] !== null &&
        ...
        if (
            jsonType.isSessionVariable &&
            jsonType.listChildElementKeys !== undefined &&
            jsonType.listChildElementKeys !== null
        ) {
    ...
    var markUnitOfRetention = function (dataTableModel) {
        ...
        for (startKey in dataTableModel) {
            ...
            elementTypePrimitive =
                jsonDefn.elementType === undefined ||
                jsonDefn.elementType === null
                    ? jsonDefn.type
                    : getPrimitiveTypeOfElementType(jsonDefn.elementType);
            if (elementTypePrimitive === "array") {
                var descendantsOfArray =
                    jsonDefn.listChildElementKeys === undefined ||
                    jsonDefn.listChildElementKeys === null
                        ? []
                        : jsonDefn.listChildElementKeys;
                var scratchArray = [];
                while (descendantsOfArray.length !== 0) {
                    ...
                    for (i = 0; i < descendantsOfArray.length; ++i) {
                        ...
                        if (jsonSubDefn !== null && jsonSubDefn !== undefined) {
                            ...
                            var listChildren =
                                jsonSubDefn.listChildElementKeys ===
                                    undefined ||
                                jsonSubDefn.listChildElementKeys === null
                                    ? []
                                    : jsonSubDefn.listChildElementKeys;
    ...
    var getDataTableModelFromSpecification = function (specification) {
        ...
        if (specification.settings.table_id.value !== "framework") {
            ...
            dataTableModel._default_access = {
                type: "string",
                isNotNullable: false,
            ...
            dataTableModel._group_privileged = {
                type: "string",
                isNotNullable: false,
            ...
            dataTableModel._group_read_only = {
                type: "string",
                isNotNullable: false,
            ...
            dataTableModel._savepoint_type = {
                type: "string",
                isNotNullable: false,
            ...
            dataTableModel._savepoint_timestamp = {
                type: "string",
                isNotNullable: true,
            ...
            dataTableModel._savepoint_creator = {
                type: "string",
                isNotNullable: false,
    ...
    var processJSONWb = function (wbJson) {
        ...
        if ("column_types" in wbJson) {
            ...
            if (userDefColumnTypes.length !== 1) {
                throw Error(
                    "Expected only one row in the 'column_types' sheet"
                );
            }
            // NOTE: we allow override of default data type definitions...
            processedColumnTypes = _.extend(
                {},
                columnTypeMap,
                userDefColumnTypes[0]
            );
        }
        ...
        if ("settings" in wbJson) {
            ...
            cleanSettingsSet = omitRowsWithMissingField(
                cleanSettingsSet,
                "setting_name"
            );
        ...
        if (processedSettings.form_id.value.length > 50) {
            throw Error(
                "The value of the 'form_id' setting_name on the settings sheet cannot be more than 50 characters long"
            );
        }

        if (processedSettings.table_id.value.length > 50) {
            throw Error(
                "The value of the 'table_id' setting_name on the settings sheet cannot be more than 50 characters long"
            );
        }

        assertValidUserDefinedName(
            "The value of the 'form_id' setting_name on the settings sheet",
            processedSettings.form_id.value
        );

        assertValidUserDefinedName(
            "The value of the 'table_id' setting_name on the settings sheet",
        ...
        if (
            _.isEmpty(processedSettings.survey.display) ||
            ((processedSettings.survey.display.title === null ||
                processedSettings.survey.display.title === undefined) &&
                processedSettings.survey.display.text !== null &&
                processedSettings.survey.display.text !== undefined)
        ) {
            throw Error(
                "Please specify the survey title under the display.title column for the 'survey' setting_name on the settings sheet"
            );
            ...
            if (_.isUndefined(form_title_text) || _.isString(form_title_text)) {
                ...
                for (var f in form_title_text) {
                    ...
                    if (
                        ...
                        translations.display === null ||
                        translations.display === undefined
                    ) {
                        locales.push({
                            display: {
                                locale: { text: f },
        ...
        if ("choices" in wbJson) {
            ...
            cleanChoicesSet = omitRowsWithMissingField(
                cleanChoicesSet,
                "choice_list_name"
            );
        ...
        if ("translations" in wbJson) {
            throw Error(
                "The sheet name 'translations' is reserved and cannot be used."
            );
        }
        ...
        if (specification.settings.table_id.value === "framework") {
            if ("table_specific_translations" in wbJson) {
                throw Error(
                    "The sheet name 'table_specific_translations' is not allowed in the framework form."
                );
            }
            // framework_translations
            {
                ...
                if ("framework_translations" in wbJson) {
                    ...
                    cleanFmkTranslationsSet = omitRowsWithMissingField(
                        cleanFmkTranslationsSet,
                        "string_token"
                    );
                    processedFmkTranslations = _.groupBy(
                        cleanFmkTranslationsSet,
                        "string_token"
                    );
                        ...
                        if (_.isArray(value)) {
                            if (value.length !== 1) {
                                throw Error(
                                    "Duplicate definitions of '" +
                                        name +
                                        "' on 'framework_translations' sheet"
                                );
                            }
                ...
                specification.framework_definitions._tokens =
                    processedFmkTranslations;
            }
            {
                var processedCmnTranslations = {};
                if ("common_translations" in wbJson) {
                    var cleanCmnTranslationsSet = wbJson.common_translations;
                    cleanCmnTranslationsSet = omitRowsWithMissingField(
                        cleanCmnTranslationsSet,
                        "string_token"
                    );
                    processedCmnTranslations = _.groupBy(
                        cleanCmnTranslationsSet,
                        "string_token"
                    );
                ...
                specification.common_definitions._tokens =
                    processedCmnTranslations;
                specification.common_definitions._locales =
                    specification.settings._locales;
                specification.common_definitions._default_locale =
                    specification.settings._default_locale;
            }
        } else if (
            specification.settings.form_id.value !==
            specification.settings.table_id.value
        ) {
            if ("framework_translations" in wbJson) {
                throw Error(
                    "The sheet name 'framework_translations' is only allowed in the framework form."
                );
            }
            if ("common_translations" in wbJson) {
                throw Error(
                    "The sheet name 'common_translations' is only allowed in the framework form."
                );
            }
            if ("table_specific_translations" in wbJson) {
                throw Error(
                    "The sheet name 'table_specific_translations' is only allowed when form_id === table_id."
                );
            }
        } else {
            if ("framework_translations" in wbJson) {
            ...
            if ("table_specific_translations" in wbJson) {
                ...
                cleanTSTranslationsSet = omitRowsWithMissingField(
                    cleanTSTranslationsSet,
                    "string_token"
                );
                processedTSTranslations = _.groupBy(
                    cleanTSTranslationsSet,
                    "string_token"
                );
                    ...
                    if (_.isArray(value)) {
                        if (value.length !== 1) {
                            throw Error(
                                "Duplicate definitions of '" +
                                    name +
                                    "' on 'table_specific_translations' sheet"
                            );
                        }
            ...
            specification.table_specific_definitions._tokens =
                processedTSTranslations;
        }
        ...
        if ("queries" in wbJson) {
            ...
            cleanQueriesSet = omitRowsWithMissingField(
                cleanQueriesSet,
                "query_name"
            );
            errorIfFieldMissing("queries", cleanQueriesSet, "query_type", true);
            errorIfFieldsMissingForQueryType(
                "queries",
                cleanQueriesSet,
                "csv",
                "uri"
        ...
        if ("calculates" in wbJson) {
            ...
            cleanCalculatesSet = omitRowsWithMissingField(
                cleanCalculatesSet,
                "calculation_name"
            );
            errorIfFieldMissing(
        ...
        if ("prompt_types" in wbJson) {
            ...
            cleanPromptTypesSet = omitRowsWithMissingField(
                cleanPromptTypesSet,
                "prompt_type_name"
            );
            errorIfFieldMissing(
        ...
        if ("model" in wbJson) {
            ...
            _.each(processedModel, function (value, key) {
                if (_.isArray(value)) {
                    ...
                    processedModel[key] = _.extend(
                        {
                            _defn: [
                                {
                                    _row_num: value[0]._row_num,
                                    section_name: "model",
                                },
                            ],
                        },
        ...
        _.each(wbJson, function (sheet, sheetName) {
            ...
            if (
                sheetName.indexOf("-") !== 0 &&
                !_.contains(reservedSheetNames, sheetName)
            ) {
        ...
        _.each(sectionNames, function (name) {
            ...
            if (!("display" in specification.settings[name])) {
                specification.settings[name].display =
                    specification.settings.survey.display;
            }
        });

        _.each(specification.settings, function (value, name) {
            sanityCheckFieldTypes(
                value,
                "settings",
        ...
        var parsedSections = parseSections(
            sectionNames,
            sections,
        ...
        _.each(specification.sections, function (section) {
            _.each(section.prompts, function (prompt) {
                if ("values_list" in prompt) {
                    ...
                    if (
                        !(
                            name in specification.choices ||
                            name in specification.queries
                        )
                    ) {
                        throw Error(
                            "Unrecognized 'values_list' name: '" +
                                name +
                                "'. Prompt: '" +
                                prompt.type +
                                "' at row " +
        ...
        specification.dataTableModel =
            getDataTableModelFromSpecification(specification);
        ...
        if (
            ...
            if ("properties" in wbJson) {
                throw Error(
                    "The 'properties' sheet is only allowed in the worksheet of the '" +
                        specification.settings.table_id.value +
                        "' form_id."
                );
            }
            ...
            if ("properties" in wbJson) {
                ...
                cleanPropertiesSet = omitRowsWithMissingField(
                    cleanPropertiesSet,
                    "partition"
                );
                cleanPropertiesSet = omitRowsWithMissingField(
                ...
                _.each(cleanPropertiesSet, function (row) {
                    ...
                    if (
                        row.partition === "Table" &&
                        row.aspect === "default" &&
                        row.key === "defaultViewType"
                    ) {
                        foundTableDefaultViewType = true;
                    }

                    if (
                        row.partition === "FormType" &&
                        row.aspect === "default" &&
                        row.key === "FormType.formType"
                    ) {
                        foundTableFormTypeSpec = true;
                    }

                    if (
                        row.partition === "SurveyUtil" &&
                        row.aspect === "default" &&
                        row.key === "SurveyUtil.formId"
                    ) {
            ...
            for (var dbColumnName in specification.dataTableModel) {
                ...
                if (
                    jsonDefn.elementSet === "data" &&
                    !jsonDefn.isSessionVariable
                ) {
                    // var surveyElementName = jsonDefn.elementName;

                    if (
                        jsonDefn.displayName !== undefined &&
                        jsonDefn.displayName !== null
                    ) {
                        processedProperties.push({
                            _partition: "Column",
                    ...
                    if (
                        jsonDefn.valuesList !== undefined &&
                        jsonDefn.valuesList !== null
                    ) {
                    ...
                    if (
                        jsonDefn.displayFormat !== undefined &&
                        jsonDefn.displayFormat !== null
                    ) {
                        processedProperties.push({
                            _partition: "Column",
                            _aspect: dbColumnName,
